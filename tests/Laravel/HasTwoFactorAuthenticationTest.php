...
it('batches saves with withoutSaving', function (): void {
    $user = TestEloquentUser::create();
    ...
    expect($refreshed->getTwoFactorSecret())->toBe('encrypted-secret')
        ->and($refreshed->getTwoFactorRecoveryCodes())->toBe('encrypted-codes')
        ->and($refreshed->getTwoFactorConfirmedAt())->toBeInstanceOf(DateTimeImmutable::class);
});
